...
- The repository-root reader remains the static-site baseline. The Windows
  desktop build embeds an independent copy under `reader/`, so Windows-only
  branding and typography do not alter the private deployed reader or root site.
...
desktop manifest switches the Windows reader copy to Note Portal labels and disables
...
3. The user opens the full rendered notice, scrolls to its end, then checks
   **I have read and agree** and selects
   **Agree and continue**.
4. A short first-run guide explains what Note Portal does, including reading
   ...
   from Help; the first-run sequence cannot be skipped and must be completed
   before library operations. Optional support information is not a condition of use.
...
- The acknowledgement checkbox remains disabled until the user opens the full
  rendered notice and scrolls to its end. **Agree and continue** then requires
  the checked box.
...
The first-run notice and short first-run guide are mandatory. The notice must
use clear, readable language and a visible decline/quit path. Agreement cannot
be inferred from merely opening, scrolling, or closing the notice; reaching
the end only enables the explicit checkbox. Guide completion is stored locally.
...
GET  /api/portal-events           # desktop reader's SSE release contract
...
- Packaged Windows-only reader copy.
