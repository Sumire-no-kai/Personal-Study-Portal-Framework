...
- 以 `.` 开头的文件与目录默认忽略；
- `.git/`、`.obsidian/`、`node_modules/`、`target/` 和 `__pycache__/`
  默认忽略；不要用这些名称存放笔记；
- `_assets/` 中的文件不成为导航项目；
- 除允许位置的 `_index.md` 外，以下划线开头的 Markdown 保留并忽略；
- `.tmp`、`.part`、`.swp`、`.bak` 及以 `~` 结尾的临时、未完成或备份
  文件与目录忽略；
- 指向资料库外部的符号链接拒绝读取；
...
32 MiB，一次扫描读取的 Markdown 总量最多 64 MiB。单个文件超出限制时会进入
诊断，其他有效笔记继续可用；总量超出时会保持上一版内容。应用不会修改原文件。
请拆分资料库或缩小文件后重试。
...
新的资料库快照只有在验证通过后才替换当前快照。单个文档暂时无法读取或解析时，
已有文档保留上一份可读正文，其他有效文档仍可更新；结构或全局 ID 冲突导致
快照不安全时，继续提供上一个有效版本。
