...
function updateDocumentTitle(view, note = null) {
  ...
  if (view === "overview") {
    document.title = `${portalData.label} | Study Portal`;
    return;
  }

  const activeNote = note || getActiveNote();
  document.title = activeNote
    ? `${activeNote.unitCode} · ${formatWeekLabel(activeNote)} | Study Portal`
    : `${portalData.label} | Study Portal`;
}
  ...
  try {
    ...
    searchResults.innerHTML = `
      <div class="search-error" role="status">
        ...
        <span>笔记阅读不受影响；请稍后重试，或检查 Study Portal 服务器。</span>
      </div>`;
